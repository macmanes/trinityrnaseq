...
#  --no_rsem                       do not make the rsem-ready files
#
    ...
    unless (-s "target.fa") {
        ...
        if ($aligner =~ /tophat|bowtie/) {
            ...
            foreach my $file (@bowtie_build_files) {
                    ...
                    my $new_filename = "target.fa." . "${rev}" . "${other_ext}.$ebwt_ext";
        ...
        if ($SS_lib_type) {
            ...
            foreach my $sam_file ("$outfile_basename.coordSorted.sam.+.sam", "$outfile_basename.coordSorted.sam.-.sam",
                ...
                if (-s $sam_file) {
                    ...
                    if ( (! $NO_RSEM) && (! $single_file) && $bam_file =~ /nameSorted/) {
...
sub run_bowtie_alignment_pipeline {
    ...
    my $cmd = "$aligner @bowtie_opts -S --sam-nohead $format target.fa $target_fa > $target_fa.pre.sam";
