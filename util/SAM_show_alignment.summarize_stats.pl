...
my $usage = "usage: $0 bwasw.sam.align_stats [longest_contig_only]\n\n";
...
while (<$fh>) {
    ...
    if (scalar (@x) == 12 && $x[3] =~ /^\d+$/) {
        ...
        $core_read_name =~ s/\.\d+\.PbioCR$// or die "Error, cannot remove suffix for $core_read_name";
        my $read_length = $x[3];
...
foreach my $core_read_acc (keys %core_acc_to_entries) {
    ...
    foreach my $alignment (@alignments) {
        ...
        if ($mismatches == 0 && $indels == 0 && $seq_length == $aligned_bases) {
